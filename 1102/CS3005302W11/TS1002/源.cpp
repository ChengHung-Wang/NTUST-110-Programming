...
bool checkWord(Table table, string word);
bool checkWord(Table table, string word, coord);
...
bool checkWord(Table table, string word, coord pos)
{
	if (pos.x < 0 || pos.y < 0 || pos.x>3 || pos.y>3 || table.position(pos).has_been)
	{
		return false;
	}
	if (!word.length())
	...
	if (word[0] == table.position(pos).alpha)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			if (checkWord(table, word.substr(1), direction[i]))
			{
				return true;
			}
		}
