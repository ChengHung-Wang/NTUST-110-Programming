...
// Check whether two position is close enough，利用 call-by-個方向隨機順序探索referce 將計算得到的方向回傳
...
//==================================================================
{
	...
	for (int direction : directions) {
		...
		if (direction == 0 || direction == 1 || direction == 2 || direction == 3) {
			if (direction == 0) {
				if (pos.y == 1 || board[pos.y - 2][pos.x] == GNOTHING) { // 走過
					continue;
				}
				...
				if (pos.y == GHEIGHT - 2 || board[pos.y + 2][pos.x] == GNOTHING) { // 走過
					continue;
				}
				board[pos.y + 1][pos.x] = GNOTHING; // change point type
				nextPos.y += 2;
				...
				if (pos.x == 1 || board[pos.y][pos.x - 2] == GNOTHING) { // 走過
					continue;
				}
				...
				if (pos.x == GWIDTH - 2 || board[pos.y][pos.x + 2] == GNOTHING) { // 走過
					continue;
				}
				board[pos.y][pos.x + 1] = GNOTHING; // change point type
				nextPos.x += 2;
